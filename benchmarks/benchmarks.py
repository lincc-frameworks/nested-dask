...
def _generate_benchmark_data(add_nested=True):
    ...
    # Convert to Dask
    base_nf = dn.NestedFrame.from_nestedpandas(base_nf).repartition(npartitions=5)
    layer_nf = dn.NestedFrame.from_nestedpandas(layer_nf).repartition(npartitions=50)
